...
import { Plus, Pencil, Trash2, X, CheckCircle, FlaskConical, Search } from 'lucide-react';
...
export default function FormulaManager() {
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      ...
      const error = err as ApiError;
      showToast(`❌ ${error.response?.data?.error || 'Error deleting formula'}`);
    } finally {
      setDeleting(null);
    }
  };
  ...
  const filtered = formulas.filter(f => {
    ...
    const matchEventType = !filterEventType || f.eventTypes.includes(filterEventType);
  ...
  return (
    <div className="space-y-6 relative">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-black text-night tracking-tight uppercase italic">Ingredient Formulas</h2>
          <p className="text-sm text-night/50 font-medium">{filtered.length} formulas</p>
        </div>
        <button
          onClick={openCreate}
      ...
      <div className="bg-white p-4 rounded-[2rem] shadow-sm border border-tumbleweed space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {(filterCategory || filterEventType || searchTerm) && (
            <button onClick={() => { setFilterCategory(''); setFilterEventType(''); setSearchTerm(''); }}
        ...
        <div className="bg-white rounded-[2rem] border border-tumbleweed/30 overflow-hidden shadow-sm">
          <table className="w-full">
            <thead>
              <tr className="border-b border-tumbleweed/20 bg-bone">
                <th className="text-left px-6 py-4 text-[10px] font-black uppercase tracking-widest text-night/40">Name</th>
                <th className="text-left px-4 py-4 text-[10px] font-black uppercase tracking-widest text-night/40">Category</th>
                <th className="text-left px-4 py-4 text-[10px] font-black uppercase tracking-widest text-night/40">Amt/Person</th>
                <th className="text-left px-4 py-4 text-[10px] font-black uppercase tracking-widest text-night/40">Packaging</th>
                <th className="text-left px-4 py-4 text-[10px] font-black uppercase tracking-widest text-night/40">Temp</th>
                <th className="text-left px-4 py-4 text-[10px] font-black uppercase tracking-widest text-night/40">Event Types</th>
                <th className="text-left px-4 py-4 text-[10px] font-black uppercase tracking-widest text-night/40">Active</th>
                ...
                <tr key={formula.id}
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <FlaskConical size={14} className="text-night/30 shrink-0" />
                      <span className="font-black text-night text-sm">{formula.name}</span>
                    </div>
                    {formula.utensil && (
                      <p className="text-[10px] text-night/30 ml-6 mt-0.5">{formula.utensil}</p>
                    )}
                  </td>
                  <td className="px-4 py-4">
                    <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-full border ${CATEGORY_COLORS[formula.category] || 'bg-gray-100 text-gray-600 border-gray-200'}`}>
                      {formula.category}
                    </span>
                  </td>
                  <td className="px-4 py-4">
                    <span className="font-black text-night text-sm">{formula.amountPerPerson}</span>
                    <span className="text-[10px] text-night/40 ml-1">{formula.unit}</span>
                  </td>
                  <td className="px-4 py-4 text-[11px] text-night/60">
                    <div>S: {formula.smallPackage} (max {formula.smallPackageMax})</div>
                    {formula.largePackage && (
                      <div className="text-night/40">L: {formula.largePackage} (max {formula.largePackageMax})</div>
                    )}
                  </td>
                  <td className="px-4 py-4">
                    <span className={`text-[9px] font-black uppercase px-2 py-1 rounded-full ${TEMP_COLORS[formula.tempType] || ''}`}>
                      {formula.tempType}
                    </span>
                  </td>
                  <td className="px-4 py-4">
                    <div className="flex flex-wrap gap-1">
                      {formula.eventTypes.map(et => (
                        <span key={et} className="text-[8px] font-black uppercase px-1.5 py-0.5 rounded bg-night/10 text-night/50">
                          {et.replace('_', ' ')}
                        </span>
                      ))}
                    </div>
                  ...
                  <td className="px-4 py-4">
                    <span className={`text-[9px] font-black uppercase px-2 py-1 rounded-full ${formula.isActive ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-400'}`}>
          ...
          {filtered.length === 0 && (
            <div className="flex flex-col items-center py-16 text-night/30">
              ...
              <p className="font-black uppercase tracking-widest text-sm">No formulas found</p>
            </div>
          )}
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-night/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
            ...
            <div className="overflow-y-auto px-8 pb-4 space-y-5 flex-1">
              ...
              {/* Name */}
              <div>
                <label className="text-[10px] font-black uppercase tracking-widest text-night/40 mb-1 block">Name *</label>
                <input
                  type="text"
                  value={form.name}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-night/40 mb-1 block">Category *</label>
                ...
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-night/40 mb-1 block">Temp Type *</label>
                  <select value={form.tempType} onChange={e => setForm(f => ({ ...f, tempType: e.target.value }))}
                    className="w-full px-4 py-3 bg-bone rounded-2xl text-sm font-bold text-night outline-none focus:ring-2 focus:ring-night cursor-pointer">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-night/40 mb-1 block">Amt / Person *</label>
                  <input type="number" step="0.01" value={form.amountPerPerson}
                    onChange={e => setForm(f => ({ ...f, amountPerPerson: parseFloat(e.target.value) || 0 }))}
                    className="w-full px-4 py-3 bg-bone rounded-2xl text-sm font-bold text-night outline-none focus:ring-2 focus:ring-night" />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-night/40 mb-1 block">Unit *</label>
                ...
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-night/40 mb-1 block">Utensil</label>
                  <input type="text" value={form.utensil}
                    onChange={e => setForm(f => ({ ...f, utensil: e.target.value }))}
                    className="w-full px-4 py-3 bg-bone rounded-2xl text-sm font-bold text-night outline-none focus:ring-2 focus:ring-night"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-night/40 mb-1 block">Small Package</label>
                  <input type="text" value={form.smallPackage}
                    onChange={e => setForm(f => ({ ...f, smallPackage: e.target.value }))}
                    className="w-full px-4 py-3 bg-bone rounded-2xl text-sm font-bold text-night outline-none focus:ring-2 focus:ring-night"
                    placeholder="e.g. Half Pan" />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-night/40 mb-1 block">Small Package Max</label>
                  <input type="number" value={form.smallPackageMax}
                    onChange={e => setForm(f => ({ ...f, smallPackageMax: parseInt(e.target.value) || 0 }))}
                    className="w-full px-4 py-3 bg-bone rounded-2xl text-sm font-bold text-night outline-none focus:ring-2 focus:ring-night" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-night/40 mb-1 block">Large Package</label>
                  <input type="text" value={form.largePackage || ''}
                    onChange={e => setForm(f => ({ ...f, largePackage: e.target.value }))}
                    className="w-full px-4 py-3 bg-bone rounded-2xl text-sm font-bold text-night outline-none focus:ring-2 focus:ring-night"
                    placeholder="e.g. Full Pan" />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-night/40 mb-1 block">Large Package Max</label>
                  <input type="number" value={form.largePackageMax || ''}
                    onChange={e => setForm(f => ({ ...f, largePackageMax: parseInt(e.target.value) || null }))}
                    className="w-full px-4 py-3 bg-bone rounded-2xl text-sm font-bold text-night outline-none focus:ring-2 focus:ring-night"
              ...
              {/* Event Types */}
              <div>
                <label className="text-[10px] font-black uppercase tracking-widest text-night/40 mb-2 block">Event Types *</label>
                <div className="flex flex-wrap gap-2">
                  {EVENT_TYPES.map(et => (
