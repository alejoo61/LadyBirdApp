...
import storesApi, { type Store } from '@/services/api/storesApi';
...
import {
  ...
  Utensils, Package, Coffee, Trash2, Edit2,
} from 'lucide-react';
...
interface BirdBox {
  id:        string;
  name:      string;   // displayName del size elegido
  tacoCount: number;
  combos:    OrderItem[];
...
const EVENT_TYPES = [
  { value: 'TACO_BAR',     label: 'Taco Bar',     icon: '🌮', desc: 'Full taco bar buffet style for large groups' },
...
// Orden de secciones por event type
const TACO_BAR_STEPS  = ['protein', 'topping', 'salsa', 'tortilla', 'snack', 'paper', 'drink'];
const BIRD_BOX_STEPS  = ['BOX_BUILDER']; // manejo especial
...
export default function NewOrderPage() {
  ...
  // ── Selecciones para Taco Bar / Personal / Fooda
  ...
  // ── Load stores
  useEffect(() => {
    storesApi.getAll().then(r => setStores(r.data.data || [])).catch(console.error);
  }, []);
  ...
  // ── Helpers de selección (Taco Bar / Personal / Fooda)
  ...
  const toggleItem = (item: MenuItem) => {
    ...
    setSelectedItems(prev => {
      ...
      if (alreadyIn && !isSingle) return filtered;
      if (alreadyIn && isSingle)  return filtered;
      return [...filtered, {
        menuItemId:  item.id,
  ...
  const saveDraftBox = () => {
    ...
    setBoxes(prev =>
      editingBoxId === 'NEW'
        ? [...prev, newBox]
        : prev.map(b => b.id === editingBoxId ? newBox : b)
    );
    setEditingBoxId(null);
    setBoxDraft({ step: 'size' });
  };

  const boxDraftItems = (): OrderItem[] => {
    const items: OrderItem[] = [];
    if (boxDraft.tortilla) items.push(boxDraft.tortilla);
    if (boxDraft.chips)    items.push(boxDraft.chips);
    items.push(...(boxDraft.combos  || []));
    items.push(...(boxDraft.addons  || []));
    if (boxDraft.paper)    items.push(boxDraft.paper);
    return items;
  };
  ...
  const setBoxDraftSingle = (item: MenuItem, field: 'tortilla' | 'chips' | 'paper') => {
    setBoxDraft(prev => {
      ...
      const same    = current?.menuItemId === item.id;
      return {
        ...prev,
    ...
    try {
      ...
      if (form.eventType === 'BIRD_BOX') {
        items = boxes.flatMap(box => {
          const boxItems = [];
          // Size/name del box
          boxItems.push({ guid: box.id, displayName: box.name, quantity: 1, price: 0, modifiers: [
          ...
          // Add-ons como line items separados
  ...
  return (
    <div className="min-h-screen bg-stone-50 flex flex-col">
      ...
      {/* ── TOP BAR ── */}
      <div className="sticky top-0 z-40 bg-white border-b border-stone-200 px-6 py-4 flex items-center justify-between">
        ...
        {infoComplete && (
          <button onClick={handleSave} disabled={saving || (form.eventType === 'BIRD_BOX' ? boxes.length === 0 : selectedItems.length === 0)}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* ── LEFT PANEL — Summary ── */}
        <div className="w-80 shrink-0 bg-stone-900 text-white flex flex-col sticky top-[65px] h-[calc(100vh-65px)] overflow-y-auto">
          ...
          <div className="flex-1 p-6 space-y-3 overflow-y-auto">
            ...
            {form.eventType === 'BIRD_BOX' && boxes.map((box, i) => (
              <div key={box.id} className="bg-white/5 rounded-xl p-3 border border-white/10">
                ...
                <p className="text-xs font-bold text-white mb-1">{box.name}</p>
                {box.combos.length > 0 && (
                  <p className="text-[10px] text-white/40">{box.combos.map(c => c.displayName).join(' / ')}</p>
                )}
                {box.tortilla && (
                  <p className="text-[10px] text-white/40">{box.tortilla.displayName}</p>
                )}
              </div>
            ))}

            {/* TACO BAR / PERSONAL / FOODA summary */}
            ...
            {infoComplete && (
              loadingMenu ? (
                ...
                <>
                      ...
                      <div className="space-y-4">
                        ...
                        {boxes.length > 0 && (
                          <div className="space-y-3">
                            {boxes.map((box, i) => (
                              <div key={box.id}
                                className="border border-stone-200 rounded-xl p-4 flex items-start justify-between gap-4 bg-white">
                                  ...
                                  <div>
                                    ...
                                    {box.combos.length > 0 && (
                                      <p className="text-xs text-stone-500 mt-0.5">
                                        Combos: {box.combos.map(c => c.displayName).join(', ')}
                                      </p>
                                    )}
                                    {box.tortilla && (
                                      <p className="text-xs text-stone-500">Tortilla: {box.tortilla.displayName}</p>
                                    )}
                                    {box.chips && (
                                      <p className="text-xs text-stone-500">Chips & Salsa: {box.chips.displayName.includes('Yes') ? 'Yes' : 'No'}</p>
                                    )}
                                    {box.addons.length > 0 && (
                                      <p className="text-xs text-stone-500">Add-ons: {box.addons.map(a => a.displayName).join(', ')}</p>
                                    )}
                                  </div>
                                </div>
                              ...
                              <BoxSection label="Box Size *" color="#0ea5e9">
                                <div className="grid grid-cols-1 gap-2">
                                  {boxSizeItems.map(item => {
                                    ...
                                    return (
                                      <button key={item.id} onClick={() => setBoxDraft(p => ({ ...p, name: item.name, tacoCount, step: 'combos' }))}
                                        className={`flex items-center justify-between px-4 py-3 rounded-xl border transition-all text-left ${
                                          selected ? 'bg-stone-900 text-white border-stone-900' : 'border-stone-200 hover:border-stone-400'}`}>
                                        <span className="text-sm font-bold">{item.name}</span>
                                        <span className={`text-[10px] font-black ${selected ? 'text-white/60' : 'text-stone-400'}`}>
                                          {getPriceLabel(item)}
                                        </span>
                                      </button>
                                    );
                                      ...
                                      return (
                                        <button key={item.id} onClick={() => setBoxDraftSingle(item, 'tortilla')}
                                          className={`flex items-center justify-between px-4 py-3 rounded-xl border transition-all text-left ${
                                            selected ? 'bg-stone-900 text-white border-stone-900' : 'border-stone-200 hover:border-stone-400'}`}>
                                          <span className="text-sm font-bold">{item.name}</span>
                                          <span className={`text-[10px] font-black ${selected ? 'text-white/60' : 'text-emerald-600'}`}>
                                            {getPriceLabel(item)}
                                          </span>
                                        </button>
                                      );
                  ...
                  {/* ─── TACO BAR / PERSONAL / FOODA PROGRESSIVE FLOW ─── */}
                              ...
                              <>
                                {canAdvance(idx) && (
                                  <button onClick={advanceStep}
                                    className="flex-1 py-3 bg-stone-900 text-white rounded-xl text-[11px] font-black uppercase tracking-widest hover:bg-red-600 transition-all flex items-center justify-center gap-2">
                                    Next: {STEP_LABELS[steps[idx + 1]] || 'Continue'}
                                    <ChevronRight size={14} />
                                  </button>
                                )}
                                {!required && !canAdvance(idx) && (
                                  <button onClick={advanceStep}
                                    className="flex-1 py-3 bg-stone-100 text-stone-500 rounded-xl text-[11px] font-black uppercase tracking-widest hover:bg-stone-200 transition-all">
                                    Skip →
                                  </button>
                                )}
                              </>
                  ...
                  {/* ─── DRINKS (siempre al final para Taco Bar) ─── */}
  ...
  return (
    <div className={`rounded-2xl border-2 overflow-hidden transition-all ${
      ...
      <div className={`flex items-center justify-between px-5 py-4 ${
        status === 'active' ? 'border-b border-stone-100' : ''
      }`}>
