// Página estática que lee el código del query string: /equipment?code=001-TP-26-01
'use client';
...
interface EquipmentData {
  id: string; equipmentCode: string; name: string; type: string;
  status: string; isDown: boolean; storeEmails: string;
  store?: { id: string; name: string; code: string; };
}
interface Store { id: string; name: string; code: string; }
type Mode = 'menu' | 'transfer' | 'maintenance' | 'success';

function EquipmentQRContent() {
  ...
  useEffect(() => {
    if (!code) { setLoading(false); setError('No equipment code provided'); return; }
    Promise.all([
      fetch(`${API_BASE}/equipment/qr/${encodeURIComponent(code)}`).then(r => r.json()),
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      if (data.success) { setSuccessMsg(isTemporary ? 'Equipment loaned!' : 'Equipment transferred!'); setMode('success'); }
  ...
  const handleMaintenanceReport = async () => {
    ...
    try {
      const res  = await fetch(`${API_BASE}/maintenance`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ equipmentId: equipment!.id, storeId: equipment!.store?.id, requestedByEmail: reporterEmail, description, urgency }),
      });
      const data = await res.json();
      if (data.success) { setSuccessMsg('Maintenance request submitted!'); setMode('success'); }
      ...
      <div className="max-w-md mx-auto">
        ...
        {mode === 'menu' && (
          <div className="space-y-3">
            <button onClick={() => setMode('transfer')}
              ...
              <div><div className="font-bold text-gray-800">Transfer / Loan</div><div className="text-sm text-gray-500">Move this equipment to another store</div></div>
            </button>
            <button onClick={() => setMode('maintenance')}
              ...
              <div><div className="font-bold text-gray-800">Report Issue</div><div className="text-sm text-gray-500">Submit a maintenance request</div></div>
            </button>
          </div>
        )}
        ...
        {mode === 'transfer' && (
          <div className="bg-white rounded-xl p-5 shadow-sm space-y-4">
            <h2 className="font-bold text-lg text-gray-800 flex items-center gap-2"><ArrowRightLeft size={20} className="text-red-600" /> Transfer / Loan</h2>
            <input value={transferredBy} onChange={e => setTransferredBy(e.target.value)} placeholder="Your name"
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-red-400" />
            <select value={toStoreId} onChange={e => setToStoreId(e.target.value)}
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-red-400">
              <option value="">Select destination store...</option>
              {stores.filter(s => s.id !== equipment?.store?.id).map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
            </select>
            <div className="flex items-center gap-3">
              <input type="checkbox" id="temporary" checked={isTemporary} onChange={e => setIsTemporary(e.target.checked)} className="w-4 h-4 rounded" />
              <label htmlFor="temporary" className="text-sm font-semibold text-gray-700">Temporary loan</label>
            </div>
            {isTemporary && <input type="date" value={returnDate} onChange={e => setReturnDate(e.target.value)}
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-red-400" />}
            <input value={transferReason} onChange={e => setTransferReason(e.target.value)} placeholder="Reason (optional)"
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-red-400" />
            {error && <p className="text-red-500 text-sm">{error}</p>}
            <div className="flex gap-3">
              <button onClick={() => { setMode('menu'); setError(''); }} className="flex-1 border border-gray-200 rounded-lg py-2 text-sm font-semibold text-gray-600">Cancel</button>
              <button onClick={handleTransfer} disabled={!toStoreId || submitting} className="flex-1 bg-red-600 text-white rounded-lg py-2 text-sm font-bold disabled:opacity-50">
                {submitting ? 'Submitting...' : isTemporary ? 'Loan' : 'Transfer'}
              </button>
            </div>
          </div>
        )}
        ...
        {mode === 'maintenance' && (
          <div className="bg-white rounded-xl p-5 shadow-sm space-y-4">
            <h2 className="font-bold text-lg text-gray-800 flex items-center gap-2"><Wrench size={20} className="text-yellow-600" /> Report Issue</h2>
            <input type="email" value={reporterEmail} onChange={e => setReporterEmail(e.target.value)} placeholder="your@email.com"
              ...
              {(['low','medium','high','critical'] as const).map(u => (
                <button key={u} onClick={() => setUrgency(u)}
                  className={`py-2 rounded-lg text-xs font-bold capitalize border-2 transition-colors ${urgency === u
                    ...
                    : 'border-gray-200 text-gray-600'}`}>{u}</button>
              ))}
            </div>
            ...
            <div className="flex gap-3">
              <button onClick={() => { setMode('menu'); setError(''); }} className="flex-1 border border-gray-200 rounded-lg py-2 text-sm font-semibold text-gray-600">Cancel</button>
              <button onClick={handleMaintenanceReport} disabled={!reporterEmail || !description || submitting} className="flex-1 bg-yellow-500 text-white rounded-lg py-2 text-sm font-bold disabled:opacity-50">
...
export default function EquipmentQRPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><Loader2 className="animate-spin text-red-600" size={40} /></div>}>
      <EquipmentQRContent />
    </Suspense>
  );
}