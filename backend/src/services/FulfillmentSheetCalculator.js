...
class FulfillmentSheetCalculator {
  ...
  async calculate(cateringOrder) {
    ...
    switch (eventType) {
      case "TACO_BAR":
        return this._calculateTacoBar(cateringOrder);
      case "BIRD_BOX":
        return this._calculateBirdBox(cateringOrder);
      case "PERSONAL_BOX":
        return this._calculatePersonalBox(cateringOrder);
      case "FOODA":
        return this._calculateFooda(cateringOrder);
      default:
        return this._calculateTacoBar(cateringOrder);
    }
  }
  ...
  async _calculateTacoBar(cateringOrder) {
    ...
    const tortillas = this._calculateTortillasFromResolved(
      ingredients,
      guestCount,
    );
  ...
  async _calculateBirdBox(cateringOrder) {
    ...
    for (const item of items) {
      ...
      if (itemNameLc.includes(SIDE_PACK_KEYWORD)) {
        ...
        if (salsaMod) {
          ...
          else if (sn.includes("patron") || sn.includes("patrón"))
            salsaName = "Salsa Patrón";
        }
        sidePacks.push({
          ...
          contents: [
            {
              item: "Guacamole",
              amount: `${32 * qty} oz`,
              packaging: `${qty}x 32 oz container`,
              utensil: "Spoon",
              tempType: "cold",
            },
            {
      ...
      if (
        itemNameLc.startsWith("chips & salsa") ||
        itemNameLc === "chips & salsa"
      ) {
        const qty = item.quantity || 1;
        const salsaMod = modifiers.find((m) => {
          const n = (m.displayName || "").toLowerCase();
      ...
      // ── Salsa Pack standalone ──
      if (
        itemNameLc.includes("salsa pack") &&
        !itemNameLc.includes(SIDE_PACK_KEYWORD)
      ) {
        const salsaMod = modifiers.find((m) => {
          const n = (m.displayName || "").toLowerCase();
          return (
      ...
      if (!hasModifiers) {
        ...
        if (canonicalName) {
          const formula = await this.resolver.getFormula(
            canonicalName,
            "BIRD_BOX",
          );
          if (formula) {
      ...
      const sizeMod = modifiers.find((m) =>
        this.resolver.isSizeModifier(m.displayName || ""),
      );
      const tacoCount = sizeMod
        ? parseInt(
            (sizeMod.displayName || "").match(/(\d+)\s*tacos?/i)?.[1] || 0,
          )
        : guestCount * 2;
      ...
      const is5050 = (tortillaMod?.displayName || "")
        .toLowerCase()
        .includes("50/50");
    ...
    const manualOtherItems = manualSalsas.filter(
      (i) => !["salsa", "addon"].includes(i.category),
    );
    ...
    const summaryItems = [
      ...
      ...addonItems.map((a) => ({
        ...
        detail:
          a.unit === "pan"
            ? `${a.quantity || 1} pan${(a.quantity || 1) > 1 ? "s" : ""}`
            : `${a.totalAmount} ${a.unit}`,
        chipsAndSalsa: "—",
        paper: "—",
      })),
      ...manualSalsaItems.map((s) => ({
        type: "salsa",
        name: s.name,
        quantity: s.quantity || 1,
        detail: `${s.totalAmount} ${s.unit}`,
        chipsAndSalsa: "—",
        paper: "—",
      })),
      ...salads.map((s) => ({
        ...
        detail: [
          s.protein
            ? s.protein.toLowerCase().includes("without") ||
              s.protein.toLowerCase().includes("no protein")
              ? "No protein"
              : s.protein
            : "No protein",
    ...
    return {
      ...
      hotItems: [
        ...
        ...sidePacks.flatMap((sp) =>
          sp.contents.filter((c) => c.tempType === "hot"),
        ),
      ],
      coldItems: [
        ...
        ...salads,
        ...sidePacks.flatMap((sp) =>
          sp.contents.filter((c) => c.tempType === "cold"),
        ),
      ],
      dryItems: [
        ...
        ...sidePacks.flatMap((sp) =>
          sp.contents.filter((c) => c.tempType === "dry"),
        ),
      ],
      proteins: [],
      toppings: [],
  ...
  async _calculateFooda(cateringOrder) {
    ...
    for (const item of items) {
      ...
      if (name.includes("chip") || name.includes("fooda")) {
        ...
        name.includes("taco") ||
        /^#\d+/.test((item.displayName || item.name || "").trim())
      ) {
        tacoRows.push({
          name: item.displayName || item.name,
    ...
    for (const item of items) {
      ...
      const proteinMod = modifiers.find((m) => {
        ...
        return (
          ...
          n.includes("chicken") ||
          n.includes("chorizo") ||
          n.includes("without protein") ||
          n.includes("no protein")
        );
      });
  ...
  _calculateTortillasFromResolved(ingredients, guestCount) {
    const result = [];
    const flourItem = ingredients.find(
      (i) => i.canonicalName === "Flour Tortillas",
    );
    const cornItem = ingredients.find(
      (i) => i.canonicalName === "Corn Tortillas",
    );
    const is5050 = flourItem && cornItem;
    if (flourItem) {
      const total = this.resolver.calculateAmount(
        flourItem.formula,
        guestCount,
      );
      const packaging = this.resolver.getPackaging(
        flourItem.formula,
        guestCount,
      );
      result.push({
